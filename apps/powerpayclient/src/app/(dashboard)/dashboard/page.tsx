...
import { BanknoteArrowDown, BanknoteArrowUp, Landmark, Bug, ExternalLink, AlertTriangle, MessageSquare, GitBranch } from "lucide-react";
...
const DashboardPage = () => {
    ...
    return (<>
        <div className="w-full h-full overflow-y-auto flex flex-col gap-4 p-4 bg-slate-300">
...
const WelcomeCard = () => {
    ...
    return (<div className="w-full h-full p-4  shsadow shadow-slate-500 rounded-md " style={{ gridArea: "welcome" }}>
        <h3 className="text-2xl flex flex-col font-bold font-[Manrope] ps-2 py-3 text-slate-500">
            <p className="text-balance">{getGreeting()},</p>
...
function getGreeting(date: Date = new Date()): string {
    ...
    const randomGreeting = variants[Math.floor(Math.random() * variants.length)];
